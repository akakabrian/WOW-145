...
/-!
# Six-vertex induced-tree constructions for WOWII Conjecture 145

This is the explicit combinatorial construction needed only in the exceptional
radius-two, diameter-four case.  It is local to the Conjecture 145 artifact and
has no dependency on the Conjecture 146 package.
-/
...
/-- A four-edge geodesic plus a vertex with exactly one neighbor on it gives a
six-vertex induced tree. -/
...
produce a six-vertex induced tree. -/
...
/-- If a diametral arm meets the length-two `z` arm before the center, an
explicit six-vertex induced tree exists. -/
lemma six_le_largestInducedTreeSize_of_cross_arm
  ...
  · subst b
    ...
    have hzaNe : z ≠ a := by
      ...
      subst a
      exact hnzy hay
    apply WOW145.Walk.six_le_largestInducedTreeSize_of_geodesic_four_add_unique_leaf
      p hp hlen (c := w) (z := z)
    · simp [p]
...
#print axioms WOW145.Walk.six_le_largestInducedTreeSize_of_geodesic_four_add_unique_leaf
#print axioms WOW145.six_le_largestInducedTreeSize_of_three_unique_insertions
#print axioms WOW145.six_le_largestInducedTreeSize_of_cross_arm
